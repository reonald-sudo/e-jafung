...
$cetakFilterInpassing = showCetakFilterInpassing("SELECT * FROM " . 'tb_inpassing' . " WHERE " . "tanggal_pengajuan >= " . "'" .  $dariTanggal . "'" . ' AND ' . 'tanggal_pengajuan <= ' . "'" . $sampaiTanggal . "'");
...
<body onload="window.print()">
    ...
    <br>
    <h4 class="tengah hitam">CETAK DATA PENGAJUAN INPASSING</h4>
    <br>
    ...
    <table class="container table table-bordered table-hover hitam">
        ...
        <tbody>
            <?php $i = 1; ?>
            <?php foreach ($cetakFilterInpassing as $row) : ?>
                <tr>
                    <td><?= $i ?></td>
                    <td><?= $row['nama']; ?></td>
                    <td><?= $row['nip']; ?></td>
