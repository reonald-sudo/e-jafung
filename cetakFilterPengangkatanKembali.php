...
$cetakFilterPengangkatanKembali = showCetakFilterPengangkatanKembali("SELECT * FROM " . 'tb_pengangkatan_kembali' . " WHERE " . "tanggal_pengajuan >= " . "'" .  $dariTanggal . "'" . ' AND ' . 'tanggal_pengajuan <= ' . "'" . $sampaiTanggal . "'");
...
<body onload="window.print()">
    ...
    <br>
    <h4 class="tengah hitam">CETAK DATA PENGAJUAN PENGANGKATAN KEMBALI</h4>
    <br>
    ...
    <table class="container table table-bordered table-hover hitam">
        ...
        <tbody>
            <?php $i = 1; ?>
            <?php foreach ($cetakFilterPengangkatanKembali as $row) : ?>
                <tr>
                    <td><?= $i ?></td>
                    <td><?= $row['nama']; ?></td>
                    <td><?= $row['nip']; ?></td>
