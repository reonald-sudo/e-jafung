...
$cetakFilterPemberhentian = showCetakFilterPemberhentian("SELECT * FROM " . 'tb_pemberhentian' . " WHERE " . "tanggal_pengajuan >= " . "'" .  $dariTanggal . "'" . ' AND ' . 'tanggal_pengajuan <= ' . "'" . $sampaiTanggal . "'");
...
<body onload="window.print()">
    ...
    <br>
    <h4 class="tengah hitam">CETAK DATA PENGAJUAN PEMBERHENTIAN JABATAN</h4>
    <br>
    ...
    <table class="container table table-bordered table-hover hitam">
        ...
        <tbody>
            <?php $i = 1; ?>
            <?php foreach ($cetakFilterPemberhentian as $row) : ?>
                <tr>
                    <td><?= $i ?></td>
                    <td><?= $row['nama']; ?></td>
                    <td><?= $row['nip']; ?></td>
