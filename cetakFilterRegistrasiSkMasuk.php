...
$cetakRegistrasiSkMasuk = query("SELECT * FROM " . 'tb_sk_masuk' . " WHERE " . "tanggal_masuk >= " . "'" .  $dariTanggal . "'" . ' AND ' . 'tanggal_masuk <= ' . "'" . $sampaiTanggal . "'");
...
<body onload="window.print()">
    ...
    <br>
    <h4 class="tengah hitam">CETAK DATA REKAP PENDATAAN SK MASUK</h4>
    <br>
    ...
    <table class="container table table-bordered table-hover hitam">
        ...
        <tbody>
            <?php $i = 1; ?>
            <?php foreach ($cetakRegistrasiSkMasuk as $row) : ?>
                <tr>
                    <td><?= $i ?></td>
                    <td><?= $row['nama']; ?></td>
                    <td><?= $row['nip']; ?></td>
