...
$cetakRegistrasiSkKeluar = query("SELECT * FROM " . 'tb_sk_keluar' . " WHERE " . "tanggal_keluar >= " . "'" .  $dariTanggal . "'" . ' AND ' . 'tanggal_keluar <= ' . "'" . $sampaiTanggal . "'");
...
<body onload="window.print()">
    ...
    <br>
    <h4 class="tengah hitam">CETAK DATA REKAP PENDATAAN SK KELUAR</h4>
    <br>
    ...
    <table class="container table table-bordered table-hover hitam">
        ...
        <tbody>
            <?php $i = 1; ?>
            <?php foreach ($cetakRegistrasiSkKeluar as $row) : ?>
                <tr>
                    <td><?= $i ?></td>
                    <td><?= $row['nama']; ?></td>
                    <td><?= $row['nip']; ?></td>
