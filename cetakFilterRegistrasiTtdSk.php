...
$cetakRegistrasiTtdSk = query("SELECT * FROM " . 'tb_registrasi_ttd_sk' . " WHERE " . "tanggal_pengajuan >= " . "'" .  $dariTanggal . "'" . ' AND ' . 'tanggal_pengajuan <= ' . "'" . $sampaiTanggal . "'");
...
<body onload="window.print()">
    ...
    <br>
    <h4 class="tengah hitam">CETAK DATA REKAP REGISTRASI TTD SK</h4>
    <br>
    ...
    <table class="container table table-bordered table-hover hitam">
        ...
        <tbody>
            <?php $i = 1; ?>
            <?php foreach ($cetakRegistrasiTtdSk as $row) : ?>
                <tr>
                    <td><?= $i ?></td>
                    <td><?= $row['nama']; ?></td>
                    <td><?= $row['nip']; ?></td>
